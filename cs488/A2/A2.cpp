...
void A2::appLogic()
{
	...
	local_x = glm::normalize( M_RT * M_scale * X_VECTOR);
  	local_y = glm::normalize( M_RT * M_scale * Y_VECTOR);
  	local_z = glm::normalize( M_RT * M_scale * Z_VECTOR);
	local_o = M_RT * ORIGIN;
	...
	vec4 vertices[6];
	for ( int i = 0; i < 6; i++ ) {
		vertices[i] = M_RT  * M_scale * octahedronVertices[i];
		vertices_view[i] = V  * vertices[i];
	}
	// Draw octahedron
