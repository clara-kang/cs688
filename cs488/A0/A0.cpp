...
void A0::resetTransformationAndColor () {
	m_shape_color = glm::vec3(1.0f, 1.0f, 1.0f);
	m_shape_translation = vec2(0.0f);
	m_shape_size = 1.0f;
	m_shape_rotation = 0.0f;
...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Reset transformation and color
		if ( ImGui::RadioButton( "Reset Transformation and Color (R)", true) ) {
		...
		// Retrieve rotation component from slider
		ImGui::SliderFloat("Rotation", &m_shape_rotation, 0.0f, 2*PI);


		ImGui::Text( "Framerate: %.1f FPS", ImGui::GetIO().Framerate );

		// Display shortcuts for scaling
		ImGui::Text( "Scale up (+), Scale down (-)");
